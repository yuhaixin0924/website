...
## 补充
### 什么是witness search
**witness search 的本质不是“完整地求 u 到 w 的最短路”，而是“在不经过 v 的前提下，尽快证明是否存在一条不比 dist(u,v)+dist(v,w) 更差的替代路径**
可以把 witness search 理解成一个“**带上界的可行性证明**”问题，而不是“完整求最短路”问题。

---

1. 它真正要回答的不是“最短多少”，而是“够不够短”

在收缩顶点 $v$ 时，考虑一对入点 $u$ 和出点 $w$。
...
> 有没有一条**不经过 $v$** 的 $u \to w$ 路径，长度 $\le L$？

注意这里的目标不是“求出最短路是多少”，而是只要证明下面两种情况之一：

- **有**：存在一条不经过 $v$ 的替代路径，且长度不超过 $L$
- **没有**：找不到这样的路径

所以 witness search 本质上是一个“是否存在 witness”的判断过程。  
这里的 witness 就是“替代路径”。
...
那就足够了，说明不用加 shortcut。

这时继续把全图都搜完，只是在浪费时间。  
所以 witness search 会做两件事：
...
情况 A：证明“有替代路径”
搜索过程中，一旦找到一条不经过 $v$ 的路径 $u \to w$，并且长度 $\le L$，就可以立即停止。
...
情况 B：证明“没有替代路径”
如果搜索在被上界 $L$ 限制的范围内，把所有可能的路径都排除了，仍然没找到长度 $\le L$ 的替代路径，那就可以认为：
- 不存在 witness
- 需要加 shortcut

所以 witness search 不是求“准确最短值”，而是在做“**是否存在足够短替代路径**”的判定。

---

4. 为什么可以剪枝

假设当前正在扩展某个中间点 $x$，从 $u$ 到 $x$ 的当前代价是 $d(u,x)$。
...
那从 $x$ 再往下走，路径只会更长，不可能再变成一条合格 witness。  
因为边权非负。

所以这种路径分支可以直接剪掉。

这就是 witness search 的“带上界搜索”：
...
那搜索结束后就知道：
- 没有 witness
- 必须加 shortcut $u \to w$，权重 7

### shortcut 是为了绕过“被收缩掉的低 rank 节点”而加的；它连接的是还没被收缩、rank 更高的两个端点。
1. shortcut 里面是不是只“压缩”了低 rank 节点？
**是的，本质上是这样。**

因为 shortcut 的作用就是把原来必须经过的低 rank 中间点折叠掉。  
所以从“展开”角度看，shortcut 代表的一条路径，其内部节点一定是在它的端点之前被收缩掉的那些点。

2. shortcut 的端点是不是也可能是低 rank？
**在标准 CH 里，一条 shortcut 连接的两个端点，通常都比它绕过的那个中间节点 rank 更高。**
但注意：

- shortcut 的端点本身也可能在后续被收缩；
- 所以一个 shortcut 最终可能又会被更高层的 shortcut 再“包一层”。

这就是 CH 里 shortcut 可以递归展开的原因。
### “向上走”不是随便规定的方向，而是预处理后证明足够的唯一搜索方向
1. 为什么不能随便向下走
向下走会回到更低 rank 的点，也就是更早被收缩的点。  
这些点之所以被收缩，就是因为它们的作用已经被 **shortcut** 提前“折叠”进图里了。
...
这些细节不是不能出现，而是**已经被 shortcut 代表掉了**，查询时没必要再展开搜索。

如果你从 `s` 开始往下走，会遇到很多低 rank 点；但这些点本来就是被收缩掉的，查询阶段再去搜它们会：

- 扩大搜索空间
- 破坏 CH 的层次单调性
- 让很多本来可以被 shortcut 代表的路径重新暴露出来

### Upward Graph不只包含shortcuts
简短要点：
定义：Upward graph 包含所有在收缩顺序上“向上”的有向边——也就是所有边 (u→v) 满足 rank(u) < rank(v) 的那些边。
内容：既包括原始图在收缩后保留并方向化的原始边（按 rank 指定方向），也包括在收缩过程中为保全最短路而插入的 shortcut（同样按 rank 方向定向）。